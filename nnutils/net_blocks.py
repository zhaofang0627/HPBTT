...
def decoder2d(nlayers, nz_shape, nc_input, use_bn=True, nc_final=1, nc_min=8, nc_step=1, init_fc=True, use_deconv=False, upconv_mode='bilinear'):
    ...
    print('texture_predictor done')
    net_init(decoder)
    return decoder


def decoder2dsep(nlayers, nz_shape, nc_input, use_bn=True, nc_final=1, nc_min=8, nc_step=1, init_fc=True, use_deconv=False, upconv_mode='bilinear'):
    ''' Simple 3D encoder with nlayers and seperated last layer.
    ...
    modules.append(nn.Conv2d(nc_output, nc_final, kernel_size=3, stride=1, padding=1, groups=6, bias=True))
    decoder = nn.Sequential(*modules)
    print('texture_predictor done')
    net_init(decoder)
    return decoder
