...
def vert_rotate(vert, deg, axis=[0, 1, 0]):
    """
    vert is B x N x 3, torch FloatTensor (or Variable)
    """
    ...
    new_vert = torch.transpose(torch.matmul(new_rot, torch.transpose(vert - center, 1, 2)), 1, 2) + center
