...
class Trainer():
    ...
    def save(self, epoch_prefix):
        '''Saves the model.'''
        self.save_network(self.model_flow, 'flow', epoch_prefix, gpu_id=self.opts.gpu_id)
        return
    ...
    def init_training(self):
        ...
        if opts.use_sgd:
            self.optimizer_flow = torch.optim.SGD(
                self.model_flow.parameters(), lr=opts.learning_rate, momentum=opts.beta1)
        else:
            self.optimizer_flow = torch.optim.Adam(
                self.model_flow.parameters(), lr=opts.learning_rate, betas=(opts.beta1, 0.999))
    ...
    def train(self):
        ...
        bg_image_data = iter(self.bg_dataloader)
        ...
        for epoch in range(opts.num_pretrain_epochs, opts.num_epochs):
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                try:
                    bg_image_batch = bg_image_data.next()
                except StopIteration:
                    bg_image_data = iter(self.bg_dataloader)
                    bg_image_batch = bg_image_data.next()

                self.set_input(batch, bg_image_batch)
                if not self.invalid_batch:
                    self.optimizer_flow.zero_grad()
                    self.forward()
                    self.smoothed_total_loss = self.smoothed_total_loss*0.99 + 0.01*self.total_loss.data.item()
