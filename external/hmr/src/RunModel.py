...
class RunModel(object):
    def __init__(self, config, sess=None):
        ...
        # Data
        self.batch_size = config.batch_size_hmr
        self.img_size = config.img_size_hmr
    ...
    def build_test_model_ief(self):
        ...
        for i in np.arange(self.num_stage):
            print('Iteration %d' % i)
            self.theta_prev.append(theta_prev)
            # ---- Compute outputs
    ...
    def load_mean_param(self):
        ...
        # self.mean_var_new = tf.Variable(
        #     mean, name="mean_param_new", dtype=tf.float32, trainable=True)
        self.E_var.append(mean)
    ...
    def predict_dict(self, images):
        ...
        fetch_dict = {
            ...
            'init_theta': self.theta_prev[0],
        }

        results = self.sess.run(fetch_dict, feed_dict)

        # print(results['init_theta'])
