...
class Rasterize(chainer.Function):
    ...
    def forward_gpu(self, inputs):
        ...
        face_index_r = self.face_index_map.copy()
        if self.return_rgb:
            rgb_r = self.rgb_map
        if self.return_alpha:
        ...
        return rgb_r, alpha_r, depth_r, face_index_r
...
def rasterize_rgbad(
    ...
    if anti_aliasing:
        # 2x super-sampling
        rgb, alpha, depth, face_index = Rasterize(
...
def rasterize(
    ...
    """
    ret = rasterize_rgbad(faces, textures, image_size, anti_aliasing, near, far, eps, background_color, True, False, False)
    return ret['rgb'], ret['face_index']
...
def rasterize_silhouettes(
    ...
    """
    ret = rasterize_rgbad(faces, None, image_size, anti_aliasing, near, far, eps, None, False, True, False)
    return ret['alpha'], ret['face_index']
