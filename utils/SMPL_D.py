...
import numpy as np
import pickle
from .util_hmr import batch_global_rigid_transformation, batch_rodrigues, batch_lrotmin, reflect_pose

# def undo_chumpy(x):
#     return x if isinstance(x, np.ndarray) else x.r

class SMPL(nn.Module):
    def __init__(self, model_path, opts, joint_type='cocoplus', obj_saveable=False):
    ...
    def forward(self, beta, theta, scale, offset, get_skin=False):
        ...
        v_shaped_unscaled = torch.matmul(beta, self.shapedirs).view(-1, self.size[0], self.size[1]) + self.v_template

        v_shaped = scale * v_shaped_unscaled + offset

        Jx = torch.matmul(v_shaped_unscaled[:, :, 0], self.J_regressor)
        Jy = torch.matmul(v_shaped_unscaled[:, :, 1], self.J_regressor)
        Jz = torch.matmul(v_shaped_unscaled[:, :, 2], self.J_regressor)
        J = scale * torch.stack([Jx, Jy, Jz], dim = 2)

        Rs = batch_rodrigues(theta.contiguous().view(-1, 3)).contiguous().view(-1, 24, 3, 3)
