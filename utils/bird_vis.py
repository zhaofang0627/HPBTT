...
from ..nnutils.nmr import NeuralRenderer, NeuralRendererWOCAM
...
class VisRenderer(object):
    ...
    def __init__(self, img_size, faces, t_size=3):
        ...
        self.renderer = NeuralRendererWOCAM(img_size)
        self.faces = Variable(
    ...
    def __call__(self, verts, cams=None, texture=None, rend_mask=False):
        ...
        if rend_mask:
            rend, _ = self.renderer.forward(verts, self.faces, cams)
            rend = rend.repeat(3, 1, 1)
            rend = rend.unsqueeze(0)
        else:
            rend, _ = self.renderer.forward(verts, self.faces, cams, texture)
